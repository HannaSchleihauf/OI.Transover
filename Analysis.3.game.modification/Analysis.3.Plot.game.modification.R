...
## Plot 3: Children's game modification
...
my_max <- function(x, na.rm = TRUE) {
  max(x, na.rm = na.rm)
}
tapply(t.data$Innovation, list(t.data$Study.population, t.data$Fidelity.type, t.data$Study.phase), my_max)
ftable(Innovation ~ Study.population + Fidelity.type + Study.phase, t.data)
...
levels(t.data$xcats)
t.data$xcats <- ordered(t.data$xcats, levels = c("first_adoption_OI","first_adoption_non-OI",
                                                 "first_transmission_OI" ,"first_transmission_non-OI", 
                                                 "second_adoption_OI", "second_adoption_non-OI", 
                                                 "second_transmission_OI", "second_transmission_non-OI"))
...
# probability to get a innovation point
t.data$prob <- t.data$Innovation / (t.data$Innovation + t.data$No.Innovation)

range(t.data$prob)

# empty plot
plot(
  x = 1, y = 1, type = "n", xlim = c(0.7, 5.3), ylim = c(-0.05, 1.05),
  bty = "l", tcl = -0.25,
  xaxs = "i", xaxt = "n",
  las = 1,
...
t.data$yvals <- t.data$prob + runif(n = nrow(t.data), min = -0.04, max = 0.04)

mycolors <- c("blue", "red", "blue4",  "red4")[as.numeric(t.data$color.id)]
...
mtext(text = "Game modification proportion", side = 2, line = 2.3, at = 0.5, cex = 0.7)

save.image("./Analysis.3.game.modification/Analysis.3.Plot.game.modification.RData")
